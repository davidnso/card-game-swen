var constants = new (function() {
    var rows = 4;//4 cards rows
    var columns = 6;//6 cards coloumn
	//total 24 cards
...
app.controller("CardController", function($scope, $timeout) {
	...
	$scope.check = function(card) {
		...
		if (numPairs == constants.getNumMatches()) {
			$scope.stopTimer();
			$scope.success = "Congradulation! you have done it.";
		}
	} //end of check()

	$scope.timeLimit = 60000;//start time limit 60s
	$scope.isCritical = false;
	$scope.success = "Matching Cards Game";
	
	var timer = null;

	$scope.start = function(){
		$scope.deck = createDeck();
		$scope.timeLimit = 60000;//time limit 60second
		...
		($scope.startTimer =function() {
			...
			if ($scope.timeLimit === 0) {
				$scope.stopTimer();
				$scope.isGuarding = true;
				$scope.success = "Too Close Good Luck.";
			}
		})();
	}	
